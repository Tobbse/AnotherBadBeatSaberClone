        ...
        **/
        public class TrackConfig
        {
            ...
            private void _createAnalyzerConfigs()
            {
                ...
                {
                    case GameController.DIFFICULTY_EASY:
                        difficultyMult = 3.0f;
                        break;

                    case GameController.DIFFICULTY_NORMAL:
                        difficultyMult = 2.5f;
                        break;

                    case GameController.DIFFICULTY_HARD:
                        difficultyMult = 2.0f;
                        break;

                    case GameController.DIFFICULTY_EXPERT:
                        difficultyMult = 1.5f;
                        break;

                    case GameController.DIFFICULTY_EXPERT_PLUS:
                        difficultyMult = 1.0f;
                        break;
            ...
            private AnalyzerBandConfig _makeConfig(int band, int startIndex, int endIndex, int thresholdSize, int beatTime, float tresholdMult)
            {
                return new AnalyzerBandConfig()
                {
                    ...
                    EndIndex = endIndex,
                    StartFrequency = _hzPerBin * startIndex,
                    EndFrequency = _hzPerBin * endIndex,
                    ThresholdSize = thresholdSize,
                    BeatBlockCounter = beatTime,
                    TresholdMult = tresholdMult
                };
            }
        }
